...
carplate_haar_cascade = cv2.CascadeClassifier('/Users/dheeraj/PycharmProjects/Number_Plate/haarcascade_russian_plate_number.xml')
...
# Searching
for img in glob.glob("Search_num/*.jpeg"):
    img = cv2.imread(img)
    print(img)
    number_plate = carplate_detect(img)
    res2 = str("".join(re.split("[^a-zA-Z0-9]*", number_plate)))
